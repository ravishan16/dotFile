...
# -----------------------------------------------------------------------------
# Script: 09_setup_ssh_github.sh
# Author: Ravishankar Sivasubramaniam
# Description: Checks for/generates SSH keys (ED25519 preferred), configures
#              SSH for GitHub, and instructs user to add public key to GitHub.
#              (Removed --apple-use-keychain from ssh-add for broader compatibility)
# -----------------------------------------------------------------------------
...
if [ -f "$ENV_FILE" ]; then
  ...
  print_message "Warning: ${ENV_FILE} not found. Will prompt for email if needed."
fi

# --- Check/Generate SSH Key ---
SSH_DIR="$HOME/.ssh"
KEY_PATH_ED25519="${SSH_DIR}/id_ed25519"
KEY_PATH_RSA="${SSH_DIR}/id_rsa" # Fallback check
KEY_TO_ADD="" # Store the path of the key we will add to the agent

mkdir -p "$SSH_DIR"
chmod 700 "$SSH_DIR"

if [ -f "$KEY_PATH_ED25519" ]; then
    print_message "Existing ED25519 SSH key found at ${KEY_PATH_ED25519}."
    PUB_KEY_PATH="${KEY_PATH_ED25519}.pub"
    KEY_TO_ADD="$KEY_PATH_ED25519"
elif [ -f "$KEY_PATH_RSA" ]; then
    print_message "Existing RSA SSH key found at ${KEY_PATH_RSA}."
    PUB_KEY_PATH="${KEY_PATH_RSA}.pub"
    KEY_TO_ADD="$KEY_PATH_RSA"
else
    print_message "No existing SSH key found. Generating a new ED25519 key..."
    ...
    echo "--------------------------------------------------------------------"; echo "⚠️ Generating SSH key without a passphrase for automation."; echo "   For higher security, generate manually with: ssh-keygen -t ed25519 -C \"${key_email}\""; echo "--------------------------------------------------------------------"
    ssh-keygen -t ed25519 -C "$key_email" -f "$KEY_PATH_ED25519" -N ""
    print_message "Generated new ED25519 key: ${KEY_PATH_ED25519}"
    PUB_KEY_PATH="${KEY_PATH_ED25519}.pub"
    KEY_TO_ADD="$KEY_PATH_ED25519"
fi
...
print_message "Configuring SSH config (${CONFIG_PATH})..."
touch "$CONFIG_PATH"; chmod 600 "$CONFIG_PATH"

# Use the determined key path in the config
...
if grep -q "Host github.com" "$CONFIG_PATH"; then print_message "SSH config entry for github.com already exists."; else
    ...
    # Add recommended settings for GitHub, ensuring IdentityFile uses the correct key
    cat << EOF >> "$CONFIG_PATH"
...
EOF
    print_message "Added github.com config entry."
fi

# --- Start ssh-agent and add key ---
print_message "Attempting to add SSH key to agent..."
eval "$(ssh-agent -s)" > /dev/null # Start agent quietly
# Use the standard ssh-add command without the macOS-specific flag
...
if [ -f "$PUB_KEY_PATH" ]; then
    ...
    echo "❌ Error: Could not find public key file at ${PUB_KEY_PATH}."
fi
